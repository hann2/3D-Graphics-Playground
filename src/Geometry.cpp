...
Geometry::Geometry(int n_vertices, int n_faces) {
    cur_face = 0;
    cur_vertex = 0;
...
void Geometry::add_vertex(float * vertex) {
    if (cur_vertex == num_vertices) {
        std::cout << "Geometry is full, cannot add vertex.\n";
...
void Geometry::add_face(int degree, int * corners) {
    if (cur_face == num_faces) {
        std::cout << "Geometry is full, cannot add face.\n";
        throw std::out_of_range("out of range");
    }
...
void Geometry::add_tex_coord(int ind, float * values) {
    memcpy(tex_coords + (ind * 2), values, 2 * sizeof(float));
}

float * Geometry::g_vertex(int ind) {
...
face_t * Geometry::g_face(int ind) {
...
float * Geometry::g_tex_coord(int ind) {
    return tex_coords + (ind * 2);
}

int Geometry::g_num_vertices() {
    return cur_vertex;
}

int Geometry::g_num_faces() {
    return cur_face;
}

float * Geometry::g_model_buffer() {
...
void Geometry::normalize_vec(float * v) {
...
float * Geometry::g_face_normal(int face_ind) {
    face_t * face = g_face(face_ind);
...
float * Geometry::g_normals_buffer() {
    if (cur_face < num_faces || cur_vertex < num_vertices) {
        std::cout << "Fill up geometry before getting normal buffer.\n";
        throw std::out_of_range("out of range");
    }

    float * vertex_normals = (float *) calloc(num_vertices * 3, sizeof(float));
...
int Geometry::g_model_buffer_size() {
    return g_num_faces() * 3 * VERTEX_SIZE;
}

int Geometry::g_normals_buffer_size() {
    return g_num_faces() * 3 * VERTEX_SIZE;
}

int Geometry::g_tex_buffer_size() {
    return g_num_faces() * 3 * 2 * sizeof(float);
}

float * Geometry::g_bounding_sphere() {
    if (cur_face < num_faces || cur_vertex < num_vertices) {
